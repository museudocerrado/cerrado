	...
	function widget( $args, $instance ) {
		...
		if ( $fields_not_empty ) {
			...
			foreach ( $fields as $index=>$field ) {
				...
				// begin field output
				echo '<p class="form-' . esc_attr($field_class) . '">' . "\n";

				echo '<span class="assistive-text">' . $title . '</span>';

				// textarea or input ?
				if ( 'message' != $index ) {

					echo '<input type="text"' . $valid_class . ' placeholder="' . $title . '" name="' . $name . '" value="" aria-required="' . $required_str . '">' . "\n";

				} else {

					echo '<textarea' . $valid_class . ' placeholder="' . $title . '" name="' . $name . '" rows="' . esc_attr( $msg_height ) . '" aria-required="' . $required_str . '"></textarea>' . "\n";
