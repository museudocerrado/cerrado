	...
	function widget( $args, $instance ) {
		...
		if ( !empty($fields) ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( !empty($field['title']) ) echo '<div class="skill-name">' . $field['title'] . '</div>';
				...
				printf(
					'<div class="skill"><div class="skill-value" data-width="%d"%s>%s</div></div>',
					$field['percent'],
					$style,
					empty($field['show_percent']) ? '' : '<span>' . $field['percent'] . '%</span>'
				);
