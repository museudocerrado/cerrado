...
function of_sanitize_upload( $input, $option = array() ) {
	...
	if ( is_array( $input ) ) {
		...
		if ( $val && is_main_site( $blog_id ) ) {
			$filetype = wp_check_filetype($val);
			if ( $filetype["ext"] ) {
				//todo check in wp folder installation

				$val = str_replace( array('http://', 'https://'), is_ssl() ? 'https://' : 'http://', $val );
				$_url = parse_url( $val );
		...
		if ( $filetype["ext"] ) {
			//todo check in wp folder installation
			// TODO: add support https for multisite

			if ( is_main_site( $blog_id ) ) {
				$input = str_replace( array('http://', 'https://'), is_ssl() ? 'https://' : 'http://', $input );
			}
