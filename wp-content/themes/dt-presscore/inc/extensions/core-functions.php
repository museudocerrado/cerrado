...
function dt_get_resized_img( $img, $opts ) {
	...
	if ( !is_array( $img ) || !$img ) return false;
	...
	if ( 1 == $opts['zc'] && $img[1] != $w && $img[2] != $h ) {
		...
		// если изображение меньше полотна
		// вычисляем пропорцию полотна
		// найти длинную сторону полотна
		// используя пропорцию находим значение меньшей стороны изображения
		// кропаем изображение
		// изображение меньше полотна
		if ( $img[1] < $w && $img[2] < $h ) {	
			// находим большую сторону полотна
			// используя пропорцию находим значение стороны изображения противоположной большей стороне полотна
			if ( $w > $h ) {
				$img_w = $img[1];
				$img_h = $img_hp;
			} else if ( $h > $w ) {
				$img_h = $img[2];
				$img_w = $img_wp;
			// квадратное полотно - кропаем по наименьшей стороне изображения
			} else {
				// ширина изображения больше или равна высоте
				if ( $img[1] >= $img[2] ) {
					$img_h = $img[2];
					$img_w = $img_wp;
				// высота изображения больше ширины
				} else {
					$img_w = $img[1];
					$img_h = $img_hp;
				}
			}
		// хоть одна сторона изображения меньше полотна
		} else if ( $img[1] < $w || $img[2] < $h ) {
			// вычисляем пропорцию полотна
			// находим сторону изображения меньшую нежели соотвествующая сторона полотна
			// используем значение меньшей стороны изображения для соответствующей стороны полотна
			// используя пропорцию вычисляем оставшуюся сторону полотна
			// кропаем изображение
 ...
 * @since presscore 0.1
 */
function dt_get_favicon( $icon = null ) {
	$output = '';
	if ( ! empty( $icon ) ) {
